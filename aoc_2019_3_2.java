...
class Scratch {
    final static Node entryNode = new Node(0, 0);
    final static List<Node> path1Nodes = new ArrayList<>();
    ...
    public static void main(String[] args) {
        ...
        assert 6 == processInput(path1, path2);
        assert 159 == processInput(path3, path4);
        assert 135 == processInput(path5, path6);
    ...
    static int processInput(String path1, String path2) {
        ...
        List<Integer> steps = intersectSet.stream()
                .map(Scratch::calculateIntersectionSteps)
                .sorted()
                .collect(Collectors.toList());
        System.out.println("Steps: " + (steps.get(0) + 2)); // +2 for the first step which we skip on each path
        return distances.iterator().next();
    }

    static int calculateManhattanDistance(Node node) {
        return Math.abs(entryNode.x - node.x) + Math.abs(entryNode.y - node.y);
    }

    static int calculateIntersectionSteps(Node node) {
        return path1Nodes.indexOf(node) + path2Nodes.indexOf(node);
    }

    private static List<Node> pathParser(String path, Node entryNode) {
        int x = entryNode.x;
        int y = entryNode.y;
